    ...
    <main className="view" id="view-docs">
      <div className="wrap doc">
        ...
        <article className="prose">
          ...
          <p>
            {brand.name} gives you every major AI model through one OpenAI-compatible endpoint. If your code already
            calls OpenAI, you only change two values.
          </p>
          <div className="call">
...
print(reply.choices[0].message.content)`}</code>
          ...
          <p>
            Send your key as a bearer token on every request. Keys belong to the wallet that created them and stop
            working the moment you revoke them.
          </p>
          <pre>
            <code>{`Authorization: Bearer ${brand.keyPrefix}...`}</code>
          </pre>
          <p>
            Never put a key in front-end code or a public repository. If a key leaks, revoke it in the dashboard and
            create a new one. {brand.name} stores only a hash of each key, so a lost key can&apos;t be shown again.
          </p>
          ...
          <p>
            ...
            <code>stream_options</code> pass through unchanged. Tool calling and JSON mode work on models whose
            provider supports them. The response&apos;s <code>model</code> field names the exact model version that
            answered. <code>n</code> must be 1.
          </p>

          <h2 id="d-models">Models</h2>
          <p>
            <span className="meth get">GET</span>
            <code>/v1/models</code> returns every model your key can use. Explorer wallets see fast models; Holder and
            Builder see the full catalog.
          </p>
          <div className="tblw">
            <table>
              <thead>
          ...
          <p>
            ...
            &quot;Runs on&quot;, this table and the catalog update, and your code keeps working without changes.
          </p>

          <h2 id="d-stream">Streaming</h2>
          <p>
            Set <code>&quot;stream&quot;: true</code> to receive server-sent events. Each event carries a small piece
            of the answer; the last one is <code>data: [DONE]</code>. Lines starting with <code>:</code> are
            keep-alive comments and can be ignored (the OpenAI SDKs do this for you).
          </p>
          <pre>
  ...
  -d '{"model": "claude-swift", "stream": true,
       "messages": [{"role": "user", "content": "Hello"}]}'`}</code>
          ...
          <p>
            ...
            <code>&quot;stream_options&quot;: {"{"}&quot;include_usage&quot;: true{"}"}</code>. The final chunk
            then carries <code>usage</code> and an empty <code>choices</code> list, so check the list before reading
            from it.
          </p>
          ...
          <p>
            Allowances reset at 00:00 UTC. Explorer and Holder responses include <code>x-refract-remaining</code> so
            your app can see how many requests are left today. A larger <code>max_tokens</code> than your tier allows
